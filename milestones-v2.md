...
Status: **M0 ratified (see §9)** — schema deltas, agent tool contract, billing config and credential inventory committed. M1 (v2 app shell) in progress.
...
## 9. M0 ratification status (committed)

M0 exit: _M0 doc committed; no open schema questions._ Status of each deliverable:

| Deliverable                                               | Status                       | Where                                                                                                                                                                                  |
| --------------------------------------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Schema deltas approved + applied                          | ✅ committed & migrated      | `prisma/schema.prisma` (§5 tables) — migration `20260903001729_v2_agent_platform`                                                                                                      |
| Stripe products ($1 invoice item, $9 promo, $29 standard) | ⏳ scripted, **not created** | `scripts/create-stripe-products.ts` + `src/lib/config.ts` (`stripe.depositPrice/pricePromo/priceStandard`, `billing.promoPriceActive/trialDays`) — run when `STRIPE_SECRET_KEY` is set |
| Platform OAuth credential inventory                       | ✅ committed                 | `docs/oauth-credential-inventory.md`                                                                                                                                                   |
| Agent tool contract                                       | ✅ committed                 | `src/lib/agent/contracts.ts` (+ `envelope.ts` permission model)                                                                                                                        |
| Promo-window flag design                                  | ✅ committed                 | `PROMO_PRICE_ACTIVE` env flag; checkout reads it at M8                                                                                                                                 |
| Roadmap marked current                                    | ✅ this section              | —                                                                                                                                                                                      |

**Applied to the database:** all §5 tables created (`AgentState`, `ConversationMessage`, `ApprovalItem`, `WorkLog`, `SyncedProfile`, `PublishedItem`, `PitchOpportunity`, `Pitch`, `RankSnapshot`, `WatchedQuery`) with cascade relations onto `User`.

**Blocked on credentials (not code):** Stripe price creation (M0 item 2) needs a live/test API key; OAuth apps for the platform tiers are inventoried and provisioned per milestone as each engine lands.
