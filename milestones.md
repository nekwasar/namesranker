...
1. Subdirectories only on `namesranker.com` — never per-user _indexed_ subdomains (authority compounding on one domain is the whole model). Exception: premium vanity links (`{name}.ra-nk.me`) are **301 redirects only**, never indexed.
...
**Convention:** _deliverable_ + _acceptance criteria_ + _estimated effort_.
